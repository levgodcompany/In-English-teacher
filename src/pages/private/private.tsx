...
const TypeLevel = lazy(() => import("./TypeLevel/TypeLevel"));
...
function Private() {
  return (
    <RoutesWithNotFound>
      <Route index element={<Navigate to={`${PrivateRoutes.TYPE_LEVEL}`} />} />
      <Route path={`${PrivateRoutes.STUDENTS}`} element={<Students />} />
      <Route path={`${PrivateRoutes.TYPE_LEVEL}`} element={<TypeLevel />} />
      <Route path={`${PrivateRoutes.LEVELS}/:idTypeLevel`} element={<Levels />} />
      <Route path={`${PrivateRoutes.COHORTS}/:idLevel`} element={<Cohorts />} />
